...
describe('AuthService', () => {
  ...
  it('deve lançar erro se usuário não encontrado', async () => {
    ...
    await expect(service.login('user@test.com', '123')).rejects.toThrow('E-mail ou senha inválida');
  });

  it('deve lançar erro se senha incorreta', async () => {
    mockFindOne.mockResolvedValue({ id: '1', senha: 'hashed', perfil: 'cliente' });
    mockCompare.mockResolvedValue(false);
    await expect(service.login('user@test.com', 'wrong')).rejects.toThrow('E-mail ou senha inválida');
  });

  it('deve retornar token e usuário no login bem-sucedido', async () => {
    const fakeUser = {
      id: 'uuid-1', nome: 'João', email: 'joao@test.com',
      senha: 'hashed', perfil: 'cliente', cpf: '123', telefone: null,
    };
